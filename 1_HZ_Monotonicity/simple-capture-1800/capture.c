...
#define CAPTURE_INTERVAL (NANOS_PER_SEC / TARGET_FPS)
...
static struct timespec time_now, time_start, time_stop, time_next_frame;
...
static void dump_ppm(const void *p, int size, unsigned int tag, struct timespec *time) {
    ...
    syslog(LOG_INFO, "PPM frame written to %s at %lf, %d bytes", ppm_dumpname, (fnow - fstart), total);
...
static void dump_pgm(const void *p, int size, unsigned int tag, struct timespec *time) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_now);
    fnow = (double)time_now.tv_sec + (double)time_now.tv_nsec / 1000000000.0;
    syslog(LOG_INFO, "PGM frame written to %s at %lf, %d bytes", pgm_dumpname, (fnow - fstart), total);
...
static void mainloop(void) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &time_next_frame);
    ...
    while (count > 0) {
        for (;;) {
            ...
            if (read_frame()) {
                ...
                time_next_frame.tv_nsec += CAPTURE_INTERVAL;
                if (time_next_frame.tv_nsec >= NANOS_PER_SEC) {
                    time_next_frame.tv_sec += 1;
                    time_next_frame.tv_nsec -= NANOS_PER_SEC;
                }

                // Sleep until the next frame capture time
                if (clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &time_next_frame, &time_error) != 0)
