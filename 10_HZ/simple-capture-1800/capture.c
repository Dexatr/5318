...
int create_directory(const char *path) {
    ...
    syslog(LOG_INFO, "Creating directory %s for frames\n", path);
    
    if (stat(path, &st) == -1) {
        if (mkdir(path, 0700) != 0) {
            syslog(LOG_ERR, "Failed to create frames directory %s: %s\n", path, strerror(errno));
            perror("Failed to create frames directory");
            return -1;
        }
    } else {
        syslog(LOG_INFO, "Directory %s already exists\n", path);
    }
...
static void dump_ppm(const void *p, int size, unsigned int tag, struct timespec *time) {
    ...
    syslog(LOG_INFO, "Preparing to dump PPM frame %04d\n", tag);
    ...
    syslog(LOG_INFO, "Writing PPM header for frame %04d\n", tag);
    ...
    if (written == -1) {
        syslog(LOG_ERR, "Failed to write ppm header for frame %04d: %s\n", tag, strerror(errno));
        perror("Failed to write ppm header");
        close(dumpfd);
        return;
    }

    syslog(LOG_INFO, "Writing PPM data for frame %04d\n", tag);

    total = 0;
    do {
        written = write(dumpfd, p, size);
        if (written == -1) {
            syslog(LOG_ERR, "Failed to write ppm data for frame %04d: %s\n", tag, strerror(errno));
    ...
    syslog(LOG_INFO, "PPM frame %04d written, %d bytes\n", tag, total);

    clock_gettime(CLOCK_MONOTONIC, &time_now);
    fnow = (double)time_now.tv_sec + (double)time_now.tv_nsec / 1000000000.0;
    syslog(LOG_INFO, "PPM frame %04d written to %s at %lf, %d bytes\n", tag, ppm_dumpname, (fnow - fstart), total);
...
static void dump_pgm(const void *p, int size, unsigned int tag, struct timespec *time) {
    ...
    syslog(LOG_INFO, "Preparing to dump PGM frame %04d\n", tag);
    ...
    syslog(LOG_INFO, "Writing PGM header for frame %04d\n", tag);
    ...
    if (written == -1) {
        syslog(LOG_ERR, "Failed to write pgm header for frame %04d: %s\n", tag, strerror(errno));
        perror("Failed to write pgm header");
        close(dumpfd);
        return;
    }

    syslog(LOG_INFO, "Writing PGM data for frame %04d\n", tag);

    total = 0;
    do {
        written = write(dumpfd, p, size);
        if (written == -1) {
            syslog(LOG_ERR, "Failed to write pgm data for frame %04d: %s\n", tag, strerror(errno));
    ...
    syslog(LOG_INFO, "PGM frame %04d written, %d bytes\n", tag, total);

    clock_gettime(CLOCK_MONOTONIC, &time_now);
    fnow = (double)time_now.tv_sec + (double)time_now.tv_nsec / 1000000000.0;
    syslog(LOG_INFO, "PGM frame %04d written to %s at %lf, %d bytes\n", tag, pgm_dumpname, (fnow - fstart), total);
...
static void mainloop(void) {
    ...
    read_delay.tv_sec = 0;
    read_delay.tv_nsec = 100000000; // 10 Hz operation with 100ms delay

    count = frame_count;

    syslog(LOG_INFO, "Starting capture loop for %d frames\n", count);

    // Main loop for capturing frames and handling I/O
    while (count > 0) {
        syslog(LOG_INFO, "Capturing frame, remaining count %d\n", count);
        for (;;) {
            fd_set fds;
            struct timeval tv;
            int r;
...
static void start_capturing(void) {
    ...
    syslog(LOG_INFO, "Starting video capturing\n");
...
static void uninit_device(void) {
    ...
    syslog(LOG_INFO, "Uninitializing device\n");
...
static void init_device(void) {
    ...
    syslog(LOG_INFO, "Initializing device\n");
...
static void open_device(void) {
    ...
    syslog(LOG_INFO, "Opening device %s\n", dev_name);
