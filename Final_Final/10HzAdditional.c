// make clean && make && sudo truncate -s 0 /var/log/syslog && ./10Hz && uname -a > 10hz_syslog.txt && sudo grep -F "[10Hz]" /var/log/syslog >> 10hz_syslog.txt
...
void create_webm(const char *frames_dir) {
    ...
    snprintf(webm_command, sizeof(webm_command), "ffmpeg -framerate %d -i %s/test%%04d.ppm -c:v libvpx -b:v 1M %s/output.webm",
             FRAMES_PER_SEC, frames_dir, frames_dir);
    int ret = system(webm_command);
    if (ret == -1) {
        syslog(LOG_ERR, "Failed to execute ffmpeg command to create WebM video [10Hz]\n");
    } else {
        syslog(LOG_INFO, "WebM video created successfully in %s [10Hz]\n", frames_dir);
    }
}
...
int main(int argc, char **argv) {
    ...
    // Combine the captured images into a WebM video
    create_webm(frames_dir);
