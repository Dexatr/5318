...
void create_webm(const char *frames_dir) {
    ...
    snprintf(webm_command, sizeof(webm_command), "ffmpeg -framerate %d -i %s/test%%04d.ppm -c:v libvpx -b:v 1M -y %s/output.webm",
    ...
    if (ret == -1) {
        ...
        syslog(LOG_ERR, "ffmpeg returned an error during WebM video creation [10Hz]\n");
    } else {
